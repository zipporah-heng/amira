...
/** Donut chart of study-type volume for "Evidence at a glance". */
export function EvidenceGlanceDonut() {
  const total = stats.studyCount;
  const R = 52;
  const C = 2 * Math.PI * R;
  let offset = 0;
  const segments = stats.studyTypes.map((t) => {
